...
def running_cost(
    state, action, state_cur, penalty_func, bbox, **kwargs
...
def main():
    ...
    save_dir = os.path.join(
        base_path, f"dump/vis/random-interact-{run_name}-model_{epoch}"
    )
    if (
        not args.resume
    ...
    bbox_2d = np.array(
        [
            [
                float(task_config["bbox"][0]) + edge_size,
                float(task_config["bbox"][1]) - edge_size,
            ],
            [
                float(task_config["bbox"][2]) + edge_size,
                float(task_config["bbox"][3]) - edge_size,
            ],
        ]

...
class Planner(object):
    def __init__(self, config):
        ...
        self.clip_action_sequences = (
            config["clip_action_seq_fn"]
            if "clip_action_seq_fn" in config
            else self.clip_actions_default
        )
        self.optimize_action_mppi = (
            config["optimize_action_mppi_fn"]
            if "optimize_action_mppi_fn" in config
            else self.optimize_action_mppi_default
        )
    ...
    def sample_action_sequences_default(self, act_seq):
        ...
        if self.noise_type == "fps":
            ...
            for i in range(self.action_dim):
                grid_axis.append(
                    np.arange(action_lower_lim_np[i], action_upper_lim_np[i], grid_size)
                )
        ...
        act_residual = torch.zeros(
            (self.n_sample, self.action_dim), dtype=act_seqs.dtype, device=self.device
        )
        ...
        for i in range(self.n_look_ahead):
            if self.noise_type == "normal":
                noise_sample = torch.normal(
                    0,
                    self.noise_level,
                    (self.n_sample, self.action_dim),
                    device=self.device,
                )
            else:
                raise ValueError("unknown noise type: %s" % (self.noise_type))

            act_residual = beta_filter * noise_sample + act_residual * (
                1.0 - beta_filter
            )
    ...
    def optimize_action_mppi_default(self, act_seqs, reward_seqs):
        act_seq = torch.sum(
            act_seqs
            * F.softmax(reward_seqs * self.reward_weight, dim=0)
            .unsqueeze(-1)
            .unsqueeze(-1),
            dim=0,
        )
    ...
    def trajectory_optimization_mppi(self, state_cur, act_seq):
        ...
        for i in range(self.n_update_iter):
            ...
            print("-" * 100)
            print(
                f"chunk: {self.chunk_id}/{self.total_chunks}, iter: {i}/{self.n_update_iter}"
            )
            with torch.no_grad():
                act_seqs = self.sample_action_sequences(
                    act_seq, iter_index=i
                )  # MPPI needs to sample actions by adding noise

                # Here to sample the action
                ...
                # NOTE(haoyang) optimize the action sequence using the reward from the current iteration
        ...
        if self.rollout_best:
            ...
            best_eval_out = self.evaluate_traj(
                best_model_out["state_seqs"], act_seq.unsqueeze(0), state_cur=state_cur
            )
    ...
    def trajectory_optimization_gd(self, state_cur, act_seq):
        act_seqs = self.sample_action_sequences(
            act_seq
        ).requires_grad_()  # (n_sample, n_look_ahead, action_dim)
    ...
    def merge_res(self, res_list):
        ...
        reward_list = [
            res["best_eval_output"]["reward_seqs"].mean().item() for res in res_list
        ]

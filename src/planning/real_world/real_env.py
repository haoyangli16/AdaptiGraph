...
class RealEnv:
    def __init__(
        ...
        if self.WRIST is not None and self.WRIST in self.serial_numbers:
            ...
            self.serial_numbers = self.serial_numbers + [
                self.WRIST
            ]  # put the wrist camera at the end
        ...
        calibration_dictionary = cv2.aruco.getPredefinedDictionary(
            cv2.aruco.DICT_6X6_250
        )
        calibration_parameters = cv2.aruco.DetectorParameters()
        self.aruco_detector = cv2.aruco.ArucoDetector(
            calibration_dictionary, calibration_parameters
        )
        ...
        if task_config is not None:
            ...
            self.clipping_height = task_config[
                "clipping_height"
            ]  # min z to prevent colliding with the table
        else:
    ...
    @property
    def is_ready(self):
        return self.realsense.is_ready and (
            self.robot.is_alive if self.use_robot else True
        )
    ...
    def step(self, action, decoded=False):
        ...
        if decoded:
            ...
            x_start, z_start, x_end, z_end = decode_action_single(
                action, push_length=self.task_config["push_length"]
            )
        ...
        if self.task_config["rotate_pusher"]:
            yaw = (
                180 - np.arctan2(z_end - z_start, x_end - x_start) / np.pi * 180
            )  # rotatable pusher
        else:
        ...
        self.reset_robot()
        time.sleep(0.5)
        self.move_to_table_position(
            [x_start, self.pusher_height + 0.15, z_start], yaw, wait=True
        )
        self.move_to_table_position(
            [x_start, self.pusher_height, z_start], yaw, wait=True
        )
        time.sleep(0.5)
        self.move_to_table_position([x_end, self.pusher_height, z_end], yaw, wait=True)
        self.move_to_table_position(
            [x_end, self.pusher_height + 0.15, z_end], yaw, wait=True
        )
        time.sleep(0.5)
        self.reset_robot()
    ...
    def step_gripper(self, action, decoded=False):
        ...
        x_start = x_start - 0.005 * (x_end - x_start) / np.sqrt(
            (x_end - x_start) ** 2 + (z_end - z_start) ** 2
        )
        z_start = z_start - 0.005 * (z_end - z_start) / np.sqrt(
            (x_end - x_start) ** 2 + (z_end - z_start) ** 2
        )
        ...
        time.sleep(0.5)
        self.move_to_table_position(
            [x_start, self.pusher_height + 0.02, z_start], yaw, wait=True
        )
        self.move_to_table_position(
            [x_end, self.pusher_height + 0.02, z_end], yaw, wait=True
        )
    ...
    def hand_eye_calibrate(self, visualize=True, save=True, return_results=True):
        ...
        for pose in poses:
            ...
            if visualize:
                cv2.imwrite(
                    f"{self.vis_dir}/calibration_handeye_img_{pose}.jpg",
                    calibration_img,
                )
            ...
            # calibrate
            corners, ids, rejected_img_points = self.aruco_detector.detectMarkers(
                calibration_img
            )
            detected_corners, detected_ids, rejected_corners, recovered_ids = (
            ...
            if visualize:
                calibration_img_vis = cv2.aruco.drawDetectedMarkers(
                    calibration_img.copy(), detected_corners, detected_ids
                )
                cv2.imwrite(
                    f"{self.vis_dir}/calibration_marker_handeye_{pose}.jpg",
                    calibration_img_vis,
                )
            ...
            if visualize:
                calibration_img_vis = calibration_img.copy()[:, :, np.newaxis].repeat(
                    3, axis=2
                )
                cv2.drawFrameAxes(calibration_img_vis, intr, dist_coef, rvec, tvec, 0.1)
                cv2.imwrite(
                    f"{self.vis_dir}/calibration_result_handeye_{pose}.jpg",
                    calibration_img_vis,
                )
            ...
            R_gripper2base.append(
                rpy_to_rotation_matrix(pose_real[3], pose_real[4], pose_real[5])
            )
        ...
        if save:
            with open(
                f"{self.calibrate_result_dir}/calibration_handeye_result.pkl", "wb"
            ) as f:
    ...
    def fixed_camera_calibrate(self, visualize=True, save=True, return_results=True):
        ...
        for i in range(self.n_fixed_cameras):  # ignore the wrist camera
            ...
            if visualize:
                cv2.imwrite(
                    f"{self.vis_dir}/calibration_img_{device}.jpg", calibration_img
                )
            ...
            if visualize:
                ...
                cv2.imwrite(
                    f"{self.vis_dir}/calibration_detected_marker_{device}.jpg",
                    calibration_img_vis,
                )
            ...
            if visualize:
                ...
                cv2.drawFrameAxes(calibration_img_vis, intr, dist_coef, rvec, tvec, 0.1)
                cv2.imwrite(
                    f"{self.vis_dir}/calibration_result_{device}.jpg",
                    calibration_img_vis,
                )
    ...
    def calibrate(self, re_calibrate=False):
        if re_calibrate:
            if self.use_robot:
                ...
                if os.path.exists(
                    f"{self.calibrate_result_dir}/calibration_handeye_result.pkl"
                ):
                    with open(
                        f"{self.calibrate_result_dir}/calibration_handeye_result.pkl",
                        "rb",
            ...
            with open(
                f"{self.calibrate_result_dir}/calibration_handeye_result.pkl", "rb"
            ) as f:
                calibration_handeye_result = pickle.load(f)
    ...
    def get_pusher_points(self):
        ...
        pusher_points_in_world = (
            R_gripper2world @ self.pusher_points.T + t_gripper2world[:, np.newaxis]
        )

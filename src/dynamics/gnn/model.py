...
class DynamicsPredictor(nn.Module):
    def __init__(self, model_config, material_config, dataset_config, device):
        ...
        material_params = material_config[dataset_config["materials"][0]][
            "physics_params"
        ]
        ...
        input_dim = (
            self.n_his * model_config["state_dim"]
        ...
        rel_input_dim = (
            model_config["rel_particle_dim"] * 2
        ...
        if model_config["verbose"]:
            ...
            print(
                "particle input dim: {}, relation input dim: {}".format(
    ...
    def forward(
        ...
        **kwargs,
        ...
        n_s = (
            attrs.size(1) - n_p
        )  # number of shape particles that do not need prediction
        ...
        state_norm_t = (
            state_norm.transpose(1, 2).contiguous().view(B, N, n_his * state_dim)
        )
        ...
        if self.model_config["rel_group_dim"] > 0:
            ...
            g = torch.cat(
                [p_instance, torch.zeros(B, n_s, n_instance).to(self.device)], 1
            )
            group_r = Rr.bmm(g)
            group_s = Rs.bmm(g)
        ...
        pred_pos = state[:, -1, :n_p] + torch.clamp(
            pred_motion, max=self.motion_clamp, min=-self.motion_clamp
        )
        if self.model_config["verbose"]:

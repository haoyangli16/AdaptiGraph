...
def train(config):
    ...
    datasets = {
        phase: DynDataset(
            dataset_config=dataset_config, material_config=material_config, phase=phase
        )
        for phase in phases
    }
    ...
    dataloaders = {
        phase: dataloader_wrapper(dataloaders[phase], phase) for phase in phases
    }
    ...
    for epoch in range(train_config["n_epochs"]):
        ...
        for phase in phases:
            with grad_manager(phase):
                ...
                loss_sum_list = []
                n_iters = (
                    train_config["n_iters_per_epoch"][phase]
                    if train_config["n_iters_per_epoch"][phase] != -1
                    else len(datasets[phase])
                )
                for i in range(n_iters):
                    ...
                    for fi in range(n_future):
                        ...
                        loss = [
                            weight * func(pred_state_p, gt_state)
        ...
        if ((epoch + 1) < 100 and (epoch + 1) % 10 == 0) or (epoch + 1) % 100 == 0:
            torch.save(
                model.state_dict(),
                os.path.join(out_dir, "checkpoints", f"model_{(epoch + 1)}.pth"),
            )
        torch.save(
            model.state_dict(), os.path.join(out_dir, "checkpoints", f"latest.pth")
        )
        torch.save(
            optimizer.state_dict(),
            os.path.join(out_dir, "checkpoints", f"latest_optim.pth"),
        )
        ...
        y_max = min(
            3 * y_min, max(max(loss_plot_list_train), max(loss_plot_list_valid))
        )
        ax.set_ylim([0, y_max])
        # save
        plt.legend()

...
class DynDataset(Dataset):
    def __init__(
        ...
        self.pair_lists, self.physics_params = load_dataset(
            dataset_config, material_config, phase
        )
    ...
    def __getitem__(self, idx):
        ...
        fps_idx_list = fps(
            obj_kp_start, self.max_nobj, self.fps_radius_range, verbose=self.verbose
        )
        ...
        eef_kp = np.stack(
            eef_kps[self.n_his - 1 : self.n_his + 1], axis=0
        )  # (2, N_eef, 3)
        ...
        state_history = np.zeros(
            (self.n_his, self.state_dim, self.pos_dim), dtype=np.float32
        )
        ...
        obj_kp_future = np.zeros(
            (self.n_future, self.obj_dim, self.pos_dim), dtype=np.float32
        )
        ...
        states_delta_future = np.zeros(
            (self.n_future - 1, self.state_dim, self.pos_dim), dtype=np.float32
        )
        eef_future = np.zeros(
            (self.n_future - 1, self.state_dim, self.pos_dim), dtype=np.float32
        )
        for fi in range(self.n_future - 1):
            eef_kp_fu = np.stack(
                eef_kps[self.n_his + fi : self.n_his + fi + 2], axis=0
            )  # (2, N_eef, 3)
            eef_future[fi, self.obj_dim : self.obj_dim + eef_kp_num] = eef_kp_fu[
                0
            ]  # (N_eef, 3)
            states_delta_future[fi, self.obj_dim : self.obj_dim + eef_kp_num] = (
                eef_kp_fu[1] - eef_kp_fu[0]
            )  # (N_eef, 3)

        if self.verbose:
            print(f"future obj states: {obj_kp_future.shape}.")
            print(
                f"future action: {states_delta_future.shape}; future eef: {eef_future.shape}."
            )
        ...
        attrs[self.max_nobj : self.max_nobj + eef_kp_num, 1] = 1.0
        ...
        for material_name in self.dataset_config["materials"]:
            if material_name not in physics_param.keys():
                raise ValueError(
                    f'Physics parameter {material_name} not found in {self.dataset_config["data_dir"]}'
                )
            physics_param[material_name] += np.random.uniform(
                -self.phys_noise,
                self.phys_noise,
        ...
        material_idx = np.zeros(
            (self.max_nobj, len(self.material_config["material_index"])), dtype=np.int32
        )
        assert (
            len(self.dataset_config["materials"]) == 1
        ), "only support single material"
        material_idx[
            :obj_kp_num,
            self.material_config["material_index"][self.dataset_config["materials"][0]],
        ] = 1

        ## add randomness
        if self.add_randomness:
            state_history += np.random.uniform(
                -self.state_noise, self.state_noise, size=state_history.shape
            )
            # rotation randomness (already translation-invariant)
            random_rot = np.random.uniform(-np.pi, np.pi)
            rot_mat = np.array(
                [
                    [np.cos(random_rot), -np.sin(random_rot), 0],
        ...
        physics_param = {
            k: torch.from_numpy(v).float() for k, v in physics_param.items()
        }
        ...
        for material_name, material_dim in self.materials.items():
            if material_name in physics_param.keys():
                ...
                graph[material_name + "_physics_param"] = torch.zeros(
                    material_dim, dtype=torch.float32
                )
